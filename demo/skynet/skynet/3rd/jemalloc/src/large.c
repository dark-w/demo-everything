                   ...
                   bool zero)
{
    ...
    if (unlikely(arena == NULL) ||
        (extent = arena_extent_alloc_large(tsdn, arena, usize, alignment,
                                           &is_zeroed)) == NULL) {
        return NULL;
    }
...
static bool large_ralloc_no_move_shrink(tsdn_t *tsdn, extent_t *extent,
                                        size_t usize)
{
    ...
    if (diff != 0) {
        extent_t *trail =
            extent_split_wrapper(tsdn, arena, &extent_hooks, extent,
...
static bool large_ralloc_no_move_expand(tsdn_t *tsdn, extent_t *extent,
                                        size_t usize, bool zero)
{
    ...
    if ((trail = extents_alloc(tsdn, arena, &extent_hooks,
                               &arena->extents_dirty, extent_past_get(extent),
                               trailsize, 0, CACHELINE, false, SC_NSIZES,
                               &is_zeroed_trail, &commit)) != NULL ||
        (trail = extents_alloc(tsdn, arena, &extent_hooks,
                               &arena->extents_muzzy, extent_past_get(extent),
                               trailsize, 0, CACHELINE, false, SC_NSIZES,
                               &is_zeroed_trail, &commit)) != NULL) {
        if (config_stats) {
            new_mapping = false;
        ...
        if ((trail = extent_alloc_wrapper(tsdn, arena, &extent_hooks,
                                          extent_past_get(extent), trailsize, 0,
                                          CACHELINE, false, SC_NSIZES,
                                          &is_zeroed_trail, &commit)) == NULL) {
            return true;
        }
        if (config_stats) {
...
static void *large_ralloc_move_helper(tsdn_t *tsdn, arena_t *arena,
 ...
 */
static void large_dalloc_prep_impl(tsdn_t *tsdn, arena_t *arena,
                                   extent_t *extent, bool junked_locked)
{
    if (!junked_locked) {
        /* See comments in arena_bin_slabs_full_insert(). */
        if (!arena_is_auto(arena)) {
...
static void large_dalloc_finish_impl(tsdn_t *tsdn, arena_t *arena,
                                     extent_t *extent)
{
