...
const char *thp_mode_names[] = { "default", "always", "never",
                                 "not supported" };
...
static void *os_pages_map(void *addr, size_t size, size_t alignment,
                          bool *commit)
{
...
static bool os_overcommits_proc(void)
{
    ...
    fd = (int)syscall(SYS_open, "/proc/sys/vm/overcommit_memory",
                      O_RDONLY | O_CLOEXEC);
#else
