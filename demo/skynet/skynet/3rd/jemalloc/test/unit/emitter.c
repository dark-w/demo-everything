...
static void forwarding_cb(void *buf_descriptor_v, const char *str)
{
    ...
    size_t written =
        malloc_snprintf(buf_descriptor->buf, buf_descriptor->len, "%s", str);
...
static void assert_emit_output(void (*emit_fn)(emitter_t *),
...
static void emit_nested_dict(emitter_t *emitter)
{
    ...
    emitter_kv(emitter, "primitive", "Another primitive", emitter_type_int,
               &val);
...
int main(void)
{
    return test_no_reentrancy(test_dict, test_table_printf, test_nested_dict,
                              test_types, test_modal, test_json_arr,
                              test_json_nested_arr, test_table_row);
}
