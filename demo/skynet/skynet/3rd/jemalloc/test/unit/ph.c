...
static void heap_print(const heap_t *heap)
{
    ...
    for (auxelm = phn_next_get(node_t, link, heap->ph_root); auxelm != NULL;
        ...
        assert_ptr_eq(phn_next_get(node_t, link,
                                   phn_prev_get(node_t, link, auxelm)),
                      auxelm, "auxelm's prev doesn't link to auxelm");
        node_print(auxelm, 0);
    }
...
static unsigned node_validate(const node_t *node, const node_t *parent)
{
    ...
    for (sibling = phn_next_get(node_t, link, leftmost_child); sibling != NULL;
         sibling = phn_next_get(node_t, link, sibling)) {
        assert_ptr_eq(phn_next_get(node_t, link,
                                   phn_prev_get(node_t, link, sibling)),
                      sibling, "sibling's prev doesn't link to sibling");
        ...
        for (j = 1; j <= NNODES; j++) {
            ...
            case 0:
                for (k = 0; k < j; k++) {
                    ...
                    assert_u_eq(heap_validate(&heap), j - k - 1,
                                "Incorrect node count");
                }
                break;
            case 1:
            ...
            case 2: {
                ...
                for (k = 0; k < j; k++) {
                    ...
                    assert_u_eq(heap_validate(&heap), j - k - 1,
                                "Incorrect node count");
                    if (prev != NULL) {
                        assert_d_ge(node_cmp(node, prev), 0,
                                    "Bad removal order");
                    }
                    prev = node;
                }
