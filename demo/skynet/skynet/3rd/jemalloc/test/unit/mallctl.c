...
TEST_BEGIN(test_mallctl_errors)
{
    ...
    assert_d_eq(mallctl("epoch", NULL, NULL, (void *)&epoch, sizeof(epoch) - 1),
                EINVAL,
...
TEST_BEGIN(test_mallctlbymib_errors)
{
    ...
    assert_d_eq(mallctlbymib(mib, miblen, NULL, NULL, "0.0.0", strlen("0.0.0")),
                EPERM,
...
TEST_BEGIN(test_mallctl_read_write)
{
    ...
    /* Read+write. */
    assert_d_eq(mallctl("epoch", (void *)&old_epoch, &sz, (void *)&new_epoch,
                        sizeof(new_epoch)),
                0, "Unexpected mallctl() failure");
    assert_zu_eq(sz, sizeof(old_epoch), "Unexpected output size");
}
...
#define TEST_MALLCTL_CONFIG(config, t)                                         \
    do {                                                                       \
        ...
        assert_d_eq(mallctl("config." #config, (void *)&oldval, &sz, NULL, 0), \
                    0, "Unexpected mallctl() failure");                        \
...
TEST_BEGIN(test_manpage_example)
{
    ...
    for (i = 0; i < nbins; i++) {
        ...
        mib[2] = i;
        len = sizeof(bin_size);
        assert_d_eq(mallctlbymib(mib, miblen, (void *)&bin_size, &len, NULL, 0),
...
#define NTCACHES 10
    ...
    for (i = 0; i < NTCACHES; i++) {
        assert_d_eq(mallctl("tcache.destroy", NULL, NULL, (void *)&tis[i],
                            sizeof(unsigned)),
...
TEST_BEGIN(test_thread_arena)
{
    ...
    if (strcmp(opa, "disabled") == 0) {
        ...
        assert_d_eq(mallctl("thread.arena", (void *)&old_arena_ind, &sz, NULL,
                            0),
                    0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_initialized)
{
    ...
    sz = sizeof(initialized);
    assert_d_eq(mallctl("arena." STRINGIFY(MALLCTL_ARENAS_ALL) ".initialized",
...
TEST_BEGIN(test_arena_i_dirty_decay_ms)
{
    ...
    assert_d_eq(mallctl("arena.0.dirty_decay_ms", (void *)&orig_dirty_decay_ms,
                        &sz, NULL, 0),
                0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_muzzy_decay_ms)
{
    ...
    assert_d_eq(mallctl("arena.0.muzzy_decay_ms", (void *)&orig_muzzy_decay_ms,
                        &sz, NULL, 0),
                0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arena_i_dss)
{
    ...
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&dss_prec_old, &sz, NULL, 0),
                0, "Unexpected mallctl() failure");
    assert_str_ne(dss_prec_old, "primary",
...
TEST_BEGIN(test_arenas_dirty_decay_ms)
{
    ...
    assert_d_eq(mallctl("arenas.dirty_decay_ms", (void *)&orig_dirty_decay_ms,
                        &sz, NULL, 0),
                0, "Unexpected mallctl() failure");
...
TEST_BEGIN(test_arenas_muzzy_decay_ms)
{
    ...
    assert_d_eq(mallctl("arenas.muzzy_decay_ms", (void *)&orig_muzzy_decay_ms,
                        &sz, NULL, 0),
                0, "Unexpected mallctl() failure");
...
#define TEST_ARENAS_BIN_CONSTANT(t, name, expected)                            \
    do {                                                                       \
        t name;                                                                \
        size_t sz = sizeof(t);                                                 \
        assert_d_eq(mallctl("arenas.bin.0." #name, (void *)&name, &sz, NULL,   \
                            0),                                                \
                    0, "Unexpected mallctl() failure");                        \
        assert_zu_eq(name, expected, "Incorrect " #name " size");              \
    } while (0)
...
#define TEST_ARENAS_LEXTENT_CONSTANT(t, name, expected)                        \
    do {                                                                       \
        t name;                                                                \
        size_t sz = sizeof(t);                                                 \
        assert_d_eq(mallctl("arenas.lextent.0." #name, (void *)&name, &sz,     \
                            NULL, 0),                                          \
                    0, "Unexpected mallctl() failure");                        \
        assert_zu_eq(name, expected, "Incorrect " #name " size");              \
    } while (0)
...
TEST_BEGIN(test_arenas_create)
{
    ...
    assert_d_eq(mallctl("arenas.narenas", (void *)&narenas_before, &sz, NULL,
                        0),
                0, "Unexpected mallctl() failure");
    assert_d_eq(mallctl("arenas.create", (void *)&arena, &sz, NULL, 0), 0,
                "Unexpected mallctl() failure");
    assert_d_eq(mallctl("arenas.narenas", (void *)&narenas_after, &sz, NULL, 0),
                0, "Unexpected mallctl() failure");
...
#define TEST_STATS_ARENAS(t, name)                                             \
    do {                                                                       \
        t name;                                                                \
        size_t sz = sizeof(t);                                                 \
        assert_d_eq(mallctl("stats.arenas.0." #name, (void *)&name, &sz, NULL, \
                            0),                                                \
                    0, "Unexpected mallctl() failure");                        \
    } while (0)
...
static void alloc_hook(void *extra, UNUSED hook_alloc_t type,
                       UNUSED void *result, UNUSED uintptr_t result_raw,
                       UNUSED uintptr_t args_raw[3])
{
    *(bool *)extra = true;
}

static void dalloc_hook(void *extra, UNUSED hook_dalloc_t type,
...
int main(void)
{
    return test(test_mallctl_errors, test_mallctlnametomib_errors,
                ...
                test_mallctlnametomib_short_mib, test_mallctl_config,
                test_mallctl_opt, test_manpage_example, test_tcache_none,
                test_tcache, test_thread_arena, test_arena_i_initialized,
                ...
                test_arenas_bin_constants, test_arenas_lextent_constants,
                test_arenas_create, test_arenas_lookup, test_stats_arenas,
                test_hooks, test_hooks_exhaustion);
}
