                    ...
                    const TValue *slot)
{
    ...
    for (loop = 0; loop < MAXTAGLOOP; loop++) {
        if (slot == NULL) { /* 't' is not a table? */
            ...
            tm = fasttm(L, hvalue(t)->metatable,
                        TM_INDEX); /* table's metamethod */
    ...
    do {
        ...
        if (!(ttisstring(s2v(top - 2)) || cvt2str(s2v(top - 2))) ||
            ...
            cast_void(tostring(L, s2v(top - 2))); /* result is first operand */
                     ...
                     s2v(top - 2))) { /* first operand is empty string? */
...
lua_Integer luaV_idiv(lua_State *L, lua_Integer m, lua_Integer n)
{
    if (l_unlikely(l_castS2U(n) + 1u <= 1u)) { /* special cases: -1 or 0 */
        ...
        if ((m ^ n) < 0 &&
            m % n != 0) /* 'm/n' would be negative non-integer? */
        ...
        vmdispatch(GET_OPCODE(i))
        {
            ...
            vmcase(OP_TAILCALL)
            {
                ...
                if (!ttisLclosure(s2v(ra))) { /* C function? */
                    ...
                    luaD_poscall(L, ci,
                                 cast_int(L->top - ra)); /* finish caller */
                ...
                if (l_unlikely(L->hookmask)) {
                    ...
                    if (nres == 0)
                        ...
                        for (; l_unlikely(nres > 1); nres--)
                            setnilvalue(
                                s2v(L->top++)); /* complete missing results */
                    }
                }
