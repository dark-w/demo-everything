...
static void singlevaraux(FuncState *fs, TString *n, expdesc *var, int base)
{
    if (fs == NULL) /* no more levels? */
        ...
        if (v >= 0) { /* found? */
            if (v == VLOCAL && !base)
                markupval(fs,
                          var->u.var.vidx); /* local will be used as an upval */
        } else { /* not found as local at current level; try upvalues */
            int idx = searchupvalue(fs, n); /* try existing upvalues */
            if (idx < 0) { /* not found? */
                singlevaraux(fs->prev, n, var, 0); /* try upper levels */
                if (var->k == VLOCAL ||
                    var->k == VUPVAL) /* local or upvalue? */
...
static Proto *addprototype(LexState *ls)
{
    ...
    if (fs->np >= f->sizep) {
        int oldsize = f->sizep;
        luaM_growvector(L, f->p, fs->np, f->sizep, Proto *, MAXARG_Bx,
                        "functions");
...
static int getlocalattribute(LexState *ls)
{
    ...
    if (testnext(ls, '<')) {
        ...
        if (strcmp(attr, "const") == 0)
            ...
            luaK_semerror(ls, luaO_pushfstring(ls->L, "unknown attribute '%s'",
                                               attr));
    }
...
static void retstat(LexState *ls)
{
    ...
    if (block_follow(ls, 1) || ls->t.token == ';')
        ...
        if (hasmultret(e.k)) {
            ...
            if (e.k == VCALL && nret == 1 &&
                !fs->bl->insidetbc) { /* tail call? */
                SET_OPCODE(getinstruction(fs, &e), OP_TAILCALL);
                lua_assert(GETARG_A(getinstruction(fs, &e)) ==
                           luaY_nvarstack(fs));
            }
