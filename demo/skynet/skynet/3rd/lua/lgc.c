...
static int remarkupvals(global_State *g)
{
    ...
    while ((thread = *p) != NULL) {
        ...
        if (!iswhite(thread) && thread->openupval != NULL)
            p = &thread
                     ->twups; /* keep marked thread with upvalues in the list */
...
static void traverseweakvalue(global_State *g, Table *h)
{
    ...
    for (n = gnode(h, 0); n < limit; n++) { /* traverse hash part */
        if (isempty(gval(n))) /* entry is empty? */
            ...
            markkey(g, n);
            if (!hasclears &&
                iscleared(g, gcvalueN(gval(n)))) /* a white value? */
...
static int traverseephemeron(global_State *g, Table *h, int inv)
{
    ...
    if (g->gcstate == GCSpropagate)
        ...
        genlink(g,
                obj2gco(h)); /* check whether collector still needs to see it */
    return marked;
}
...
static void separatetobefnz(global_State *g, int all)
{
    ...
    while ((curr = *p) !=
           g->finobjold1) { /* traverse all finalizable objects */
    ...
    if (g->lastatomic != 0) /* last collection was a bad one? */
        ...
        if (g->GCdebt > 0 && gettotalbytes(g) > majorbase + majorinc) {
           ...
           collection; keep doing minor collections */
                ...
                g->lastatomic =
                    numobjs; /* signal that last collection was bad */
