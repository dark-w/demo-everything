...
static void add_node(struct timer *T, struct timer_node *node)
{
    ...
    if ((time | TIME_NEAR_MASK) == (current_time | TIME_NEAR_MASK)) {
        ...
        link(&T->t[i][((time >> (TIME_NEAR_SHIFT + i * TIME_LEVEL_SHIFT)) &
                       TIME_LEVEL_MASK)],
             node);
    }
}
...
static void timer_add(struct timer *T, void *arg, size_t sz, int time)
{
    struct timer_node *node =
        (struct timer_node *)skynet_malloc(sizeof(*node) + sz);
...
uint64_t skynet_thread_time(void)
{
    ...
    return (uint64_t)ti.tv_sec * MICROSEC +
           (uint64_t)ti.tv_nsec / (NANOSEC / MICROSEC);
}
