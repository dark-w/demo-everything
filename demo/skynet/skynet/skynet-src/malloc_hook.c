...
static ssize_t *get_allocated_field(uint32_t handle)
{
...
inline static void *fill_prefix(char *ptr)
{
    ...
    struct mem_cookie *p =
        (struct mem_cookie *)(ptr + size - sizeof(struct mem_cookie));
...
int mallctl_opt(const char *name, int *newval)
{
    ...
    if (newval) {
        ...
        if (ret == 0) {
            ...
            skynet_error(NULL,
                         "set new value(%d) for (%s) failed: error -> %d\n",
                         *newval, name, ret);
        }
    } else {
        je_mallctl(name, &v, &len, NULL, 0);
...
{
    void *ptr = je_aligned_alloc(alignment,
                                 size + (size_t)((PREFIX_SIZE + alignment - 1) &
                                                 ~(alignment - 1)));
    if (!ptr)
        malloc_oom(size);
    return fill_prefix(ptr);
...
void dump_c_mem()
{
    ...
    for (i = 0; i < SLOT_SIZE; i++) {
        ...
        if (data->handle != 0 && data->allocated != 0) {
            ...
            skynet_error(NULL, ":%08x -> %zdkb %db", data->handle,
                         data->allocated >> 10, (int)(data->allocated % 1024));
        }
    }
