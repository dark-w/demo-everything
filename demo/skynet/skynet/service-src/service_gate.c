...
static void _forward_agent(struct gate *g, int fd, uint32_t agentaddr,
                           uint32_t clientaddr)
{
...
static void _forward(struct gate *g, struct connection *c, int size)
{
    ...
    if (g->broker) {
        ...
        skynet_send(ctx, 0, g->broker, g->client_tag | PTYPE_TAG_DONTCOPY, fd,
                    temp, size);
        return;
    }
    if (c->agent) {
        ...
        skynet_send(ctx, c->client, c->agent,
                    g->client_tag | PTYPE_TAG_DONTCOPY, fd, temp, size);
    } else if (g->watchdog) {
        char *tmp = skynet_malloc(size + 32);
        int n = snprintf(tmp, 32, "%d data ", c->id);
        databuffer_read(&c->buffer, &g->mp, tmp + n, size);
        skynet_send(ctx, 0, g->watchdog, PTYPE_TEXT | PTYPE_TAG_DONTCOPY, fd,
                    tmp, size + n);
    }
}
                                    ...
                                    int sz)
{
    ...
    case SKYNET_SOCKET_TYPE_DATA: {
        ...
        if (id >= 0) {
            ...
            skynet_error(ctx, "Drop unknown connection %d message",
                         message->id);
    ...
    case SKYNET_SOCKET_TYPE_ACCEPT:
        ...
        if (hashid_full(&g->hash)) {
            ...
            struct connection *c =
                &g->conn[hashid_insert(&g->hash, message->ud)];
    ...
    case SKYNET_SOCKET_TYPE_WARNING:
        skynet_error(ctx, "fd (%d) send buffer (%d)K", message->id,
                     message->ud);
        break;
    }
}
               ...
               uint32_t source, const void *msg, size_t sz)
{
    ...
    case PTYPE_CLIENT: {
        ...
        uint32_t uid =
            idbuf[0] | idbuf[1] << 8 | idbuf[2] << 16 | idbuf[3] << 24;
        int id = hashid_lookup(&g->hash, uid);
        if (id >= 0) {
    ...
    case PTYPE_SOCKET:
        ...
        dispatch_socket_message(
            g, msg, (int)(sz - sizeof(struct skynet_socket_message)));
        break;
    }
    return 0;
}
...
int gate_init(struct gate *g, struct skynet_context *ctx, char *parm)
{
    ...
    int client_tag = 0;
    char header;
    int n = sscanf(parm, "%c %s %s %d %d", &header, watchdog, binding,
                   &client_tag, &max);
    if (n < 4) {
