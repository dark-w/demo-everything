...
int luaopen_ltls_c(lua_State *L)
{
    if (!TLS_IS_INIT) {
        luaL_error(L,
                   "ltls need init, Put enablessl = true in you config file.");
    }
    luaL_Reg l[] = {
        { "newctx", lnew_ctx },
        { "newtls", lnew_tls },
...
// for ltls init
static int ltls_init_constructor(lua_State *L)
{
...
static int ltls_init_destructor(lua_State *L)
{
#ifndef OPENSSL_EXTERNAL_INITIALIZATION
    if (TLS_IS_INIT) {
        ENGINE_cleanup();
...
int luaopen_ltls_init_c(lua_State *L)
{
    luaL_Reg l[] = {
