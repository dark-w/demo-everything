...
static int countsize(lua_State *L, int sizearray)
{
    int n = 0;
    lua_pushnil(L);
...
static uint32_t calchash(const char *str, size_t l)
{
...
static int stringindex(struct context *ctx, const char *str, size_t sz)
{
    lua_State *L = ctx->L;
...
static void setvalue(struct context *ctx, lua_State *L, int index,
                     struct node *n)
{
...
static void setarray(struct context *ctx, lua_State *L, int index, int key)
{
    struct node n;
...
static int ishashkey(struct context *ctx, lua_State *L, int index, int *key,
                     uint32_t *keyhash, int *keytype)
{
...
static void fillnocolliding(lua_State *L, struct context *ctx)
{
    struct table *tbl = ctx->tbl;
    lua_pushnil(L);
    while (lua_next(L, 1) != 0) {
...
static void fillcolliding(lua_State *L, struct context *ctx)
{
    struct table *tbl = ctx->tbl;
...
static void delete_tbl(struct table *tbl)
{
    int i;
...
static void convert_stringmap(struct context *ctx, struct table *tbl)
{
    lua_State *L = ctx->L;
...
static int lnewconf(lua_State *L)
{
    int ret;
    struct context ctx;
...
static struct table *get_table(lua_State *L, int index)
{
...
static int ldeleteconf(lua_State *L)
{
...
static void pushvalue(lua_State *L, lua_State *sL, uint8_t vt, union value *v)
{
    switch (vt) {
    case VALUETYPE_REAL:
...
static struct node *lookup_key(struct table *tbl, uint32_t keyhash, int key,
                               int keytype, const char *str, size_t sz)
{
    if (tbl->sizehash == 0)
        return NULL;
    struct node *n = &tbl->hash[keyhash % tbl->sizehash];
...
static int lindexconf(lua_State *L)
{
    struct table *tbl = get_table(L, 1);
    int kt = lua_type(L, 2);
    uint32_t keyhash;
...
static void pushkey(lua_State *L, lua_State *sL, struct node *n)
{
    if (n->keytype == KEYTYPE_INTEGER) {
...
static int pushfirsthash(lua_State *L, struct table *tbl)
{
    if (tbl->sizehash) {
...
static int releaseobj(lua_State *L)
{
...
static int lboxconf(lua_State *L)
{
    struct table *tbl = get_table(L, 1);
    struct state *s = lua_touserdata(tbl->L, 1);
    ATOM_FINC(&s->ref);
...
static int lmarkdirty(lua_State *L)
{
    struct table *tbl = get_table(L, 1);
    struct state *s = lua_touserdata(tbl->L, 1);
    s->dirty = 1;
...
static int lgetref(lua_State *L)
{
    struct table *tbl = get_table(L, 1);
    struct state *s = lua_touserdata(tbl->L, 1);
...
static int lincref(lua_State *L)
{
    struct table *tbl = get_table(L, 1);
    struct state *s = lua_touserdata(tbl->L, 1);
...
static int ldecref(lua_State *L)
{
    struct table *tbl = get_table(L, 1);
    struct state *s = lua_touserdata(tbl->L, 1);
...
static int lneedupdate(lua_State *L)
{
    struct ctrl *c = lua_touserdata(L, 1);
    if (c->update) {
...
LUAMOD_API int luaopen_skynet_sharedata_core(lua_State *L)
{
    luaL_Reg l[] = {
        // used by host
        { "new", lnewconf },
