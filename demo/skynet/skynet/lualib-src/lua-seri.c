...
static void rball_init(struct read_block *rb, char *buffer, int size)
{
    rb->buffer = buffer;
    rb->len = size;
    rb->ptr = 0;
...
static void *rb_read(struct read_block *rb, int sz)
{
    if (rb->len < sz) {
        return NULL;
    }
...
static inline void wb_pointer(struct write_block *wb, void *v)
{
...
static int wb_table_array(lua_State *L, struct write_block *wb, int index,
                          int depth)
{
...
static void wb_table_hash(lua_State *L, struct write_block *wb, int index,
                          int depth, int array_size)
{
    lua_pushnil(L);
...
static void wb_table_metapairs(lua_State *L, struct write_block *wb, int index,
                               int depth)
{
...
static void pack_from(lua_State *L, struct write_block *b, int from)
{
...
static inline void invalid_stream_line(lua_State *L, struct read_block *rb,
                                       int line)
{
    int len = rb->len;
...
static lua_Integer get_integer(lua_State *L, struct read_block *rb, int cookie)
{
    switch (cookie) {
...
static void *get_pointer(lua_State *L, struct read_block *rb)
{
    void *userdata = 0;
...
static void unpack_table(lua_State *L, struct read_block *rb, int array_size)
{
    if (array_size == MAX_COOKIE - 1) {
        uint8_t type;
...
static void push_value(lua_State *L, struct read_block *rb, int type,
                       int cookie)
{
    switch (type) {
    case TYPE_NIL:
...
LUAMOD_API int luaseri_pack(lua_State *L)
{
    struct block temp;
    temp.next = NULL;
