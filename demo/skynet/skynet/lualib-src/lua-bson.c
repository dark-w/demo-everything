...
static inline int32_t get_length(const uint8_t *data)
{
...
static inline void bson_destroy(struct bson *b)
{
    if (b->ptr != b->buffer) {
        free(b->ptr);
    }
...
static inline void bson_create(struct bson *b)
{
    b->size = 0;
    b->cap = DEFAULT_CAP;
    b->ptr = b->buffer;
...
static inline void bson_reserve(struct bson *b, int sz)
{
    if (b->size + sz <= b->cap)
        return;
    do {
...
static inline void check_reader(lua_State *L, struct bson_reader *br, int sz)
{
    if (br->size < sz) {
...
static inline int read_byte(lua_State *L, struct bson_reader *br)
{
    check_reader(L, br, 1);
...
static inline int32_t read_int32(lua_State *L, struct bson_reader *br)
{
    check_reader(L, br, 4);
    const uint8_t *b = br->ptr;
...
static inline int64_t read_int64(lua_State *L, struct bson_reader *br)
{
    check_reader(L, br, 8);
    const uint8_t *b = br->ptr;
...
static inline lua_Number read_double(lua_State *L, struct bson_reader *br)
{
    check_reader(L, br, 8);
    union {
        uint64_t i;
...
static inline const void *read_bytes(lua_State *L, struct bson_reader *br,
                                     int sz)
{
    const void *r = br->ptr;
...
static inline const char *read_cstring(lua_State *L, struct bson_reader *br,
                                       size_t *sz)
{
    int i;
    for (i = 0;; i++) {
...
static inline void write_byte(struct bson *b, uint8_t v)
{
    bson_reserve(b, 1);
    b->ptr[b->size++] = v;
}

static inline void write_int32(struct bson *b, int32_t v)
{
...
static inline void write_length(struct bson *b, int32_t v, int off)
{
    uint32_t uv = (uint32_t)v;
    b->ptr[off++] = uv & 0xff;
    b->ptr[off++] = (uv >> 8) & 0xff;
...
static int utf8_copy(const char *s, char *d, size_t limit)
{
...
static void write_string(struct bson *b, lua_State *L, const char *key,
                         size_t sz)
{
    bson_reserve(b, sz + 1);
...
static inline int reserve_length(struct bson *b)
{
    int sz = b->size;
    bson_reserve(b, 4);
    b->size += 4;
...
static inline void write_int64(struct bson *b, int64_t v)
{
...
static inline void write_double(struct bson *b, lua_Number d)
{
    union {
        double d;
        uint64_t i;
...
static inline void append_key(struct bson *bs, lua_State *L, int type,
...
static inline int is_32bit(int64_t v)
{
    return v >= INT32_MIN && v <= INT32_MAX;
}

static void append_number(struct bson *bs, lua_State *L, const char *key,
                          size_t sz)
{
...
static void append_table(struct bson *bs, lua_State *L, const char *key,
                         size_t sz, int depth);

static void write_binary(struct bson *b, const void *buffer, size_t sz)
{
...
static void append_one(struct bson *bs, lua_State *L, const char *key,
                       size_t sz, int depth)
{
    int vt = lua_type(L, -1);
    switch (vt) {
...
static inline int bson_numstr(char *str, unsigned int i)
{
    if (i < MAX_NUMBER) {
...
static void pack_array(lua_State *L, struct bson *b, int depth, size_t len)
{
    int length = reserve_length(b);
    size_t i;
    for (i = 1; i <= len; i++) {
...
static void pack_dict_data(lua_State *L, struct bson *b, int depth, int kt)
{
    const char *key = NULL;
    size_t sz;
    switch (kt) {
...
static void pack_simple_dict(lua_State *L, struct bson *b, int depth)
{
    int length = reserve_length(b);
    lua_pushnil(L);
...
static void pack_meta_dict(lua_State *L, struct bson *b, int depth)
{
    int length = reserve_length(b);
...
static bool is_rawarray(lua_State *L)
{
...
static void make_object(lua_State *L, int type, const void *ptr, size_t len)
{
    luaL_Buffer b;
    luaL_buffinit(L, &b);
    luaL_addchar(&b, 0);
...
static int lmakeindex(lua_State *L)
{
...
static void replace_object(lua_State *L, int type, struct bson *bs)
{
    size_t len = 0;
...
static void bson_meta(lua_State *L)
{
...
static int encode_bson(lua_State *L)
{
...
static int encode_bson_byorder(lua_State *L)
{
    int n = lua_gettop(L);
...
static int lencode_order(lua_State *L)
{
    struct bson b;
    int n = lua_gettop(L);
    if (n % 2 != 0) {
...
static int lbinary(lua_State *L)
{
    lua_settop(L, 1);
    luaL_Buffer b;
    luaL_buffinit(L, &b);
...
static int lsubtype(lua_State *L, int subtype, const uint8_t *buf, size_t sz)
{
    switch (subtype) {
    case BSON_BINARY:
...
static int ltype(lua_State *L)
{
    ...
    case LUA_TSTRING: {
        ...
        const char *str = lua_tolstring(L, 1, &len);
        if (str[0] == 0 && len >= 2) {
            return lsubtype(L, (uint8_t)str[1], (const uint8_t *)str + 2,
                            len - 2);
        } else {
            type = 5;
            break;
...
static int lobjectid(lua_State *L)
{
...
LUAMOD_API int luaopen_bson(lua_State *L)
{
    ...
    luaL_Reg l[] = {
        { "encode", lencode },     { "encode_order", lencode_order },
        { "date", ldate },         { "timestamp", ltimestamp },
        { "regex", lregex },       { "binary", lbinary },
        { "objectid", lobjectid }, { "int64", lint64 },
        { "decode", ldecode },     { NULL, NULL },
    };
