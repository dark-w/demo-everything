...
static int lfreepool(lua_State *L)
{
...
static int lnewpool(lua_State *L, int sz)
{
    struct buffer_node *pool =
        lua_newuserdatauv(L, sizeof(struct buffer_node) * sz, 0);
    int i;
    for (i = 0; i < sz; i++) {
        pool[i].msg = NULL;
        pool[i].sz = 0;
...
static int lpushbuffer(lua_State *L)
{
    ...
    struct buffer_node *free_node =
        lua_touserdata(L, -1); // sb poolt msg size free_node
    lua_pop(L, 1);
    if (free_node == NULL) {
...
static void pop_lstring(lua_State *L, struct socket_buffer *sb, int sz,
                        int skip)
{
    ...
    for (;;) {
        ...
        int real_sz = sz - skip;
        if (real_sz > 0) {
            luaL_addlstring(&b, current->msg + sb->offset,
                            (real_sz < bytes) ? real_sz : bytes);
        }
        return_free_node(L, 2, sb);
        sz -= bytes;
        if (sz == 0)
...
static bool check_sep(struct buffer_node *node, int from, const char *sep,
                      int seplen)
{
    for (;;) {
        int sz = node->sz - from;
...
static const char *address_port(lua_State *L, char *tmp, const char *addr,
...
static void get_buffer(lua_State *L, int index, struct socket_sendbuffer *buf)
{
    void *buffer;
...
static int lsendlow(lua_State *L)
{
    struct skynet_context *ctx = lua_touserdata(L, lua_upvalueindex(1));
    int id = luaL_checkinteger(L, 1);
    struct socket_sendbuffer buf;
...
static void getinfo(lua_State *L, struct socket_info *si)
{
    lua_newtable(L);
...
LUAMOD_API int luaopen_skynet_socketdriver(lua_State *L)
{
    luaL_checkversion(L);
    luaL_Reg l[] = {
        { "buffer", lnewbuffer },  { "push", lpushbuffer },
        { "pop", lpopbuffer },     { "drop", ldrop },
        { "readall", lreadall },   { "clear", lclearbuffer },
        { "readline", lreadline }, { "str2p", lstr2p },
        { "header", lheader },     { "info", linfo },

        { "unpack", lunpack },     { NULL, NULL },
    };
    luaL_newlib(L, l);
    luaL_Reg l2[] = {
        { "connect", lconnect },
