...
function CMD.online(client)
    ...
    if roles[name] then
        ...
        sendto(
            client.fd,
            ("范围变为 [%d - %d], 接下来由 %s 来操作"):format(game.down_limit, game.up_limit, game.turns[game.user_turn])
        )
    end
    skynet.retpack()
end

local function game_over(name)
    print(name)
    for _, role in pairs(roles) do
        local client_tmp = read_table(skynet.call(redisd, "lua", "hgetall", "role:" .. role.name))
...
function CMD.guess(name, val)
    ...
    if val == game.random_value then
        broadcast(("游戏结束，%s猜中了数字%d"):format(name, val))
        game_over(name)
        return
    end
    if val < game.random_value then
