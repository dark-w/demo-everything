...
local function game_over(score)
    ...
    rds:hset("role:"..client.name, "score", score)
end
...
-- TODO: return a entire rank table
function CMD.rank()
    local score = client.score

    --- redis operation
    local all_clients = rds:keys("*")
    local all_scores = {}
    for index, client_name in pairs(all_clients) do
        local client_tmp = rds:hgetall(client_name)
        rank[index] = client_tmp
        print(rank[index].name .. " "  .. " " .. rank[index].score)
    end
