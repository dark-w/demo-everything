...
static int str_rep(lua_State *L)
{
    ...
    if (n <= 0)
        ...
        memcpy(p, s,
               l * sizeof(char)); /* last copy (not followed by separator) */
...
static int str_format(lua_State *L)
{
    ...
    while (strfrmt < strfrmt_end) {
        if (*strfrmt != L_ESC)
            ...
            char *buff =
                luaL_prepbuffsize(&b, maxitem); /* to put formatted item */
...
static int str_pack(lua_State *L)
{
    ...
    while (*fmt != '\0') {
        ...
        case Kuint: { /* unsigned integers */
            ...
            if (size < SZINT) /* need overflow check? */
                luaL_argcheck(
                    L, (lua_Unsigned)n < ((lua_Unsigned)1 << (size * NB)), arg,
                    "unsigned overflow");
                             ...
                             int size, int issigned)
{
    ...
    if (size < SZINT) { /* real size smaller than lua_Integer? */
        ...
        for (i = limit; i < size; i++) {
            if (l_unlikely((unsigned char)str[islittle ? i : size - 1 - i] !=
                           mask))
