...
static void test_bitmap_initializer_body(const bitmap_info_t *binfo,
                                         size_t nbits)
{
...
#else
    assert_zu_eq(
        binfo->ngroups, binfo_dyn.ngroups,
        "Unexpected difference between static and dynamic initialization");
#endif
}
...
#undef NB
    ...
    static size_t test_bitmap_size_body(const bitmap_info_t *binfo,
                                        size_t nbits, size_t prev_size)
{
    ...
    if (nbits >= 3) {
        for (size_t i = 0; i < nbits - 2; i++) {
            ...
            assert_zu_eq(bitmap_ffu(bitmap, binfo, i + 2), i + 2,
                         "Unexpected first unset bit");
            if (i + 3 < nbits) {
                assert_zu_eq(bitmap_ffu(bitmap, binfo, i + 3), nbits,
                             "Unexpected first unset bit");
            }
            assert_zu_eq(bitmap_sfu(bitmap, binfo), i,
                         "Unexpected first unset bit");
