...
static unsigned do_arena_create(ssize_t dirty_decay_ms, ssize_t muzzy_decay_ms)
{
    ...
    assert_d_eq(mallctlbymib(mib, miblen, NULL, NULL, (void *)&dirty_decay_ms,
    ...
    assert_d_eq(mallctlnametomib("arena.0.muzzy_decay_ms", mib, &miblen), 0,
                "Unexpected mallctlnametomib() failure");
    mib[1] = (size_t)arena_ind;
    assert_d_eq(mallctlbymib(mib, miblen, NULL, NULL, (void *)&muzzy_decay_ms,
...
TEST_BEGIN(test_decay_ticks)
{
    ...
    sz = sizeof(size_t);
    assert_d_eq(mallctl("arenas.lextent.0.size", (void *)&large0, &sz, NULL, 0),
    ...
    assert_d_eq(mallctl("thread.arena", (void *)&old_arena_ind, &sz_arena_ind,
    ...
    for (i = 0; i < sizeof(tcache_sizes) / sizeof(size_t); i++) {
        ...
        tick0 = ticker_read(decay_ticker);
        assert_d_eq(mallctl("tcache.flush", NULL, NULL, (void *)&tcache_ind,
                            sizeof(unsigned)),
                    0, "Unexpected mallctl failure");
        tick1 = ticker_read(decay_ticker);
...
static void decay_ticker_helper(unsigned arena_ind, int flags, bool dirty,
                                ssize_t dt, uint64_t dirty_npurge0,
                                uint64_t muzzy_npurge0, bool terminate_asap)
{
#define NINTERVALS 101
    ...
    do {
        ...
        nstime_compare(&time, &deadline) <= 0 &&
        ((dirty_npurge1 == dirty_npurge0 && muzzy_npurge1 == muzzy_npurge0) ||
         !terminate_asap));
    dallocx(p, flags);

    if (config_stats) {
        assert_u64_gt(dirty_npurge1 + muzzy_npurge1,
                      dirty_npurge0 + muzzy_npurge0,
...
int main(void)
{
    return test(test_decay_ticks, test_decay_ticker, test_decay_nonmonotonic,
                test_decay_now, test_decay_never);
}
