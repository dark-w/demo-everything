...
TEST_BEGIN(huge_bind_thread)
{
    ...
    assert_d_eq(mallctl("thread.arena", NULL, NULL, &arena1, sizeof(arena1)), 0,
                "Fail to bind thread");

    void *ptr = mallocx(HUGE_SZ, 0);
    assert_ptr_not_null(ptr, "Fail to allocate huge size");
    assert_d_eq(mallctl("arenas.lookup", &arena2, &sz, &ptr, sizeof(ptr)), 0,
    ...
    arena2 = 0;
    assert_d_eq(mallctl("thread.arena", NULL, NULL, &arena2, sizeof(arena2)), 0,
                "Fail to bind thread");
...
TEST_BEGIN(huge_mallocx)
{
    ...
    assert_d_eq(mallctl("arenas.lookup", &arena2, &sz, &huge, sizeof(huge)), 0,
                "Unexpected mallctl() failure");
    ...
    assert_ptr_not_null(huge, "Fail to allocate huge size");
    assert_d_eq(mallctl("arenas.lookup", &arena2, &sz, &huge2, sizeof(huge2)),
