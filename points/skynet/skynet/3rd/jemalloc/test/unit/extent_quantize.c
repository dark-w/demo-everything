...
TEST_BEGIN(test_small_extent_size)
{
    ...
    for (i = 0; i < nbins; i++) {
        mib[2] = i;
        sz = sizeof(size_t);
        assert_d_eq(mallctlbymib(mib, miblen, (void *)&extent_size, &sz, NULL,
                                 0),
...
TEST_BEGIN(test_large_extent_size)
{
    ...
    sz = sizeof(bool);
    assert_d_eq(mallctl("config.cache_oblivious", (void *)&cache_oblivious, &sz,
                        NULL, 0),
                0, "Unexpected mallctl failure");

    sz = sizeof(unsigned);
    assert_d_eq(mallctl("arenas.nlextents", (void *)&nlextents, &sz, NULL, 0),
                0, "Unexpected mallctl failure");
    ...
    for (i = 0; i < nlextents; i++) {
        ...
        mib[2] = i;
        sz = sizeof(size_t);
        assert_d_eq(mallctlbymib(mib, miblen, (void *)&lextent_size, &sz, NULL,
                                 0),
                    0, "Unexpected mallctlbymib failure");
...
#define SZ_MAX ZU(4 * 1024 * 1024)
    ...
    for (i = 1; i <= SZ_MAX >> LG_PAGE; i++) {
        ...
        assert_zu_le(
            floor, extent_size,
            "Floor should be <= (floor=%zu, extent_size=%zu, ceil=%zu)", floor,
            extent_size, ceil);
