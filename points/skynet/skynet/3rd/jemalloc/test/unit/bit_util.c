...
#define TEST_POW2_CEIL(t, suf, pri)                                            \
    do {                                                                       \
        ...
        for (i = 0; i < sizeof(t) * 8; i++) {                                  \
            assert_##suf##_eq(pow2_ceil_##suf(((t)1) << i), ((t)1) << i,       \
                              "Unexpected result");                            \
        }                                                                      \
                                                                               \
        for (i = 2; i < sizeof(t) * 8; i++) {                                  \
            assert_##suf##_eq(pow2_ceil_##suf((((t)1) << i) - 1), ((t)1) << i, \
                              "Unexpected result");                            \
        }                                                                      \
                                                                               \
        ...
        for (pow2 = 1; pow2 < 25; pow2++) {                                    \
            for (x = (((t)1) << (pow2 - 1)) + 1; x <= ((t)1) << pow2; x++) {   \
                assert_##suf##_eq(pow2_ceil_##suf(x), ((t)1) << pow2,          \
...
void assert_lg_ceil_range(size_t input, unsigned answer)
{
    ...
    assert_zu_le(input, (ZU(1) << answer), "Got %u as lg_ceil of %zu", answer,
                 input);
    assert_zu_gt(input, (ZU(1) << (answer - 1)), "Got %u as lg_ceil of %zu",
                 answer, input);
}
...
void assert_lg_floor_range(size_t input, unsigned answer)
{
    ...
    assert_zu_ge(input, (ZU(1) << answer), "Got %u as lg_floor of %zu", answer,
                 input);
    assert_zu_lt(input, (ZU(1) << (answer + 1)), "Got %u as lg_floor of %zu",
                 answer, input);
}
...
int main(void)
{
    return test(test_pow2_ceil_u64, test_pow2_ceil_u32, test_pow2_ceil_zu,
                test_lg_ceil_floor);
}
