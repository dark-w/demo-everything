                               ...
                               bool *commit, unsigned arena_ind)
{
    ...
    ret = default_hooks->alloc(default_hooks, new_addr, size, alignment, zero,
                               commit, 0);
    did_alloc = (ret != NULL);
    return ret;
}
                                     ...
                                     unsigned arena_ind)
{
    ...
    err = default_hooks->purge_forced == NULL ||
          default_hooks->purge_forced(default_hooks, addr, size, offset, length,
                                      0);
