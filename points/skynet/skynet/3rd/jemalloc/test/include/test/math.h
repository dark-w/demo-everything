...
static inline double pt_norm(double p)
{
    ...
    if (fabs(q) <= 0.425) {
        ...
        r = 0.180625 - q * q;
        return q *
               (((((((2.5090809287301226727e3 * r + 3.3430575583588128105e4) *
                         r +
    ...
    if (df < -1.24 * log(p)) {
        ...
        if (df > 0.32) {
            ...
            while (true) {
                ...
                ch -= (1.0 -
                       exp(a + ln_gamma_df_2 + 0.5 * ch + c * aa) * p2 / p1) /
                      t;
 ...
 * p.
 */
static inline double pt_gamma(double p, double shape, double scale,
