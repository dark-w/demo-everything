...
TEST_BEGIN(test_overflow)
{
    ...
    assert_ptr_null(smallocx(ZU(PTRDIFF_MAX) + 1, 0).ptr,
                    "Expected OOM for smallocx(size=%#zx, 0)",
                    ZU(PTRDIFF_MAX) + 1);
...
static void *remote_alloc(void *arg)
{
    ...
    size_t large_sz;
    sz = sizeof(size_t);
    assert_d_eq(mallctl("arenas.lextent.0.size", (void *)&large_sz, &sz, NULL,
                        0),
...
#define MAXSZ (((size_t)1) << 23)
    ...
    for (sz = 1; sz < MAXSZ; sz = nallocx(sz, 0) + 1) {
        ...
        p = ret.ptr;
        smz = ret.size;
        assert_ptr_not_null(p, "Unexpected smallocx(size=%zx, flags=0) error",
                            sz);
        rsz = sallocx(p, 0);
