...
TEST_BEGIN(test_slab_sizes)
{
    ...
    len = 4;
    assert_d_eq(mallctlnametomib("arenas.bin.0.slab_size", slabmib, &len), 0,
    ...
    for (unsigned i = 0; i < nbins; i++) {
        ...
        sizemib[2] = i;
        slabmib[2] = i;
        assert_d_eq(mallctlbymib(sizemib, 4, (void *)&bin_size, &len, NULL, 0),
                    0, "bin size mallctlbymib failure");

        len = sizeof(size_t);
        assert_d_eq(mallctlbymib(slabmib, 4, (void *)&slab_size, &len, NULL, 0),
