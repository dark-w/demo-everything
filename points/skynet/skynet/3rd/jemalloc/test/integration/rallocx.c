...
static size_t get_size_impl(const char *cmd, size_t ind)
{
    ...
    assert_d_eq(mallctlbymib(mib, miblen, (void *)&ret, &z, NULL, 0), 0,
                "Unexpected mallctlbymib([\"%s\", %zu], ...) failure", cmd,
                ind);
    ...
    for (i = 0; i < NCYCLES; i++) {
        ...
        for (j--; j > 0; j--) {
            ...
            assert_ptr_not_null(q,
                                ...
                                szs[j], szs[j - 1]);
            tsz = sallocx(q, 0);
            assert_zu_eq(tsz, szs[j - 1], "Expected size=%zu, got size=%zu",
                         szs[j - 1], tsz);
            p = q;
        }
    }
...
#define RANGE 2048
    ...
    for (i = 0; i < sizeof(start_sizes) / sizeof(size_t); i++) {
        ...
        for (j = 1; j < RANGE; j++) {
            ...
            qsz = sallocx(q, 0);
            if (q != p || qsz != psz) {
                assert_false(validate_fill(q, FILL_BYTE, 0, psz),
                             "Expected filled memory");
...
#define MAX_ALIGN (ZU(1) << 25)
    ...
    for (align <<= 1; align <= MAX_ALIGN; align <<= 1) {
        ...
        assert_ptr_not_null(q, "Unexpected rallocx() error for align=%zu",
                            align);
        assert_ptr_null((void *)((uintptr_t)q & (align - 1)),
...
#define MAX_VALIDATE (ZU(1) << 22)
    ...
    for (lg_align++; lg_align <= MAX_LG_ALIGN; lg_align++) {
        ...
        assert_ptr_not_null(q, "Unexpected rallocx() error for lg_align=%u",
                            lg_align);
        ...
        if ((sz << 1) <= MAX_VALIDATE) {
            ...
            assert_false(validate_fill(q, 0, 0, MAX_VALIDATE),
                         "Expected zeroed memory");
            assert_false(
                validate_fill((void *)((uintptr_t)q + sz - MAX_VALIDATE), 0, 0,
                              MAX_VALIDATE),
                "Expected zeroed memory");
        }
        p = q;
...
TEST_BEGIN(test_overflow)
{
    ...
    assert_ptr_null(rallocx(p, ZU(PTRDIFF_MAX) + 1, 0),
                    "Expected OOM for rallocx(p, size=%#zx, 0)",
                    ZU(PTRDIFF_MAX) + 1);
