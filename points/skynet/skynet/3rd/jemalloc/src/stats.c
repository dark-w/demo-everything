...
static void mutex_stats_init_cols(
    emitter_row_t *row, const char *table_name, emitter_col_t *name,
...
static void mutex_stats_read_global(
...
#define OP(counter, counter_type, human, derived, base_counter)                \
    ...
    if (!derived) {                                                            \
        gen_mutex_ctl_str(cmd, MUTEX_CTL_STR_MAX_LENGTH, "mutexes", name,      \
                          #counter);                                           \
...
static void mutex_stats_read_arena(
    unsigned arena_ind, mutex_prof_arena_ind_t mutex_ind, const char *name,
...
#define OP(counter, counter_type, human, derived, base_counter)                \
    ...
    if (!derived) {                                                            \
        ...
        CTL_M2_GET(cmd, arena_ind, (counter_type *)&dst->bool_val,             \
                   counter_type);                                              \
    } else {                                                                   \
        emitter_col_t *base =                                                  \
            &col_##counter_type[mutex_counter_##base_counter];                 \
                                   ...
                                   uint64_t uptime)
{
    ...
    for (j = 0, in_gap = false; j < nbins; j++) {
        ...
        CTL_M2_M4_GET("stats.arenas.0.bins.0.nrequests", i, j, &nrequests,
                      uint64_t);
                                       ...
                                       uint64_t uptime)
{
    ...
    for (j = 0, in_gap = false; j < nlextents; j++) {
        ...
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.nmalloc", i, j, &nmalloc,
                      uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.ndalloc", i, j, &ndalloc,
                      uint64_t);
        CTL_M2_M4_GET("stats.arenas.0.lextents.0.nrequests", i, j, &nrequests,
                      uint64_t);
        in_gap_prev = in_gap;
        in_gap = (nrequests == 0);
...
static void stats_arena_extents_print(emitter_t *emitter, unsigned i)
{
    ...
    for (j = 0; j < SC_NPSIZES; j++) {
        ...
        CTL_M2_M4_GET("stats.arenas.0.extents.0.nretained", i, j, &nretained,
                      size_t);
                              ...
                              bool large, bool mutex, bool extents)
{
    ...
    emitter_kv(emitter, "dss", "dss allocation precedence", emitter_type_string,
               &dss);
...
#undef OPT_WRITE_CHAR_P
    ...
    if (config_prof) {
        ...
        emitter_kv(emitter, "interval", "prof.interval", emitter_type_uint64,
                   &u64v);

        CTL_GET("prof.lg_sample", &ssv, ssize_t);
        emitter_kv(emitter, "lg_sample", "prof.lg_sample", emitter_type_ssize,
                   &ssv);
                               ...
                               bool extents)
{
    ...
    emitter_table_printf(emitter,
                         ...
                         "num_runs: %" FMTu64 ", run_interval: %" FMTu64
                         " ns\n",
        ...
        if (unmerged) {
            for (i = j = 0; i < narenas; i++) {
                if (initialized[i]) {
                    ...
                    malloc_snprintf(arena_ind_str, sizeof(arena_ind_str), "%u",
                                    i);
