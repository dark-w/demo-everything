                            ...
                            szind_t binind, unsigned rem)
{
    ...
    while (nflush > 0) {
        ...
        for (unsigned i = 0; i < nflush; i++) {
            ...
            if (extent_arena_ind_get(extent) == bin_arena_ind &&
                ...
                arena_dalloc_bin_junked_locked(tsd_tsdn(tsd), bin_arena, bin,
                                               binind, extent, ptr);
            } else {
                /*
    ...
    memmove(tbin->avail - rem, tbin->avail - tbin->ncached,
            rem * sizeof(void *));
    tbin->ncached = rem;
                            ...
                            unsigned rem, tcache_t *tcache)
{
    ...
    while (nflush > 0) {
        ...
        if ((config_prof || config_stats) && (locked_arena == tcache_arena)) {
            if (config_prof) {
                idump = arena_prof_accum(tsd_tsdn(tsd), tcache_arena,
                                         tcache->prof_accumbytes);
                tcache->prof_accumbytes = 0;
            }
            if (config_stats) {
...
static void tcache_destroy(tsd_t *tsd, tcache_t *tcache, bool tsd_tcache)
{
    ...
    if (tsd_tcache) {
        ...
        void *avail_array =
            (void *)((uintptr_t)tcache_small_bin_get(tcache, 0)->avail -
                     (uintptr_t)tcache_bin_info[0].ncached_max *
                         sizeof(void *));
	 ...
	 */
    arena_decay(tsd_tsdn(tsd), arena_get(tsd_tsdn(tsd), 0, false), false,
                false);
...
static bool tcaches_create_prep(tsd_t *tsd)
{
    ...
    if (tcaches == NULL) {
        tcaches = base_alloc(tsd_tsdn(tsd), b0get(),
                             sizeof(tcache_t *) * (MALLOCX_TCACHE_MAX + 1),
                             CACHELINE);
        if (tcaches == NULL) {
            err = true;
            goto label_return;
