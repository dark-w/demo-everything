...
static const ctl_named_node_t *ctl_named_children(const ctl_named_node_t *node,
                                                  size_t index)
{
...
#define INDEX_PROTO(n)                                                         \
    static const ctl_named_node_t *n##_index(tsdn_t *tsdn, const size_t *mib,  \
...
static const ctl_indexed_node_t stats_arenas_i_bins_node[] = { { INDEX(
...
static const ctl_indexed_node_t stats_arenas_i_lextents_node[] = { { INDEX(
...
static const ctl_indexed_node_t stats_arenas_i_extents_node[] = { { INDEX(
    stats_arenas_i_extents_j) } };
...
static const ctl_indexed_node_t stats_arenas_node[] = { { INDEX(
    stats_arenas_i) } };
...
static const ctl_named_node_t super_root_node[] = { { NAME(""),
                                                      CHILD(named, root) } };
 ...
 */
static void ctl_accum_arena_stats_u64(arena_stats_u64_t *dst,
                                      arena_stats_u64_t *src)
{
...
static void ctl_arena_stats_amerge(tsdn_t *tsdn, ctl_arena_t *ctl_arena,
                                   arena_t *arena)
{
    unsigned i;
...
static void ctl_arena_stats_sdmerge(ctl_arena_t *ctl_sdarena,
                                    ctl_arena_t *ctl_arena, bool destroyed)
{
    unsigned i;
...
#undef OP
        if (!destroyed) {
            ...
            assert(atomic_load_zu(&astats->astats.internal, ATOMIC_RELAXED) ==
                   0);
        }
...
static void ctl_arena_refresh(tsdn_t *tsdn, arena_t *arena,
                              ctl_arena_t *ctl_sdarena, unsigned i,
                              bool destroyed)
{
...
#define READ_GLOBAL_MUTEX_PROF_DATA(i, mtx)                                    \
    ...
    malloc_mutex_unlock(tsdn, &mtx);
        ...
        if (have_background_thread) {
            READ_GLOBAL_MUTEX_PROF_DATA(global_prof_mutex_background_thread,
...
static bool ctl_init(tsd_t *tsd)
{
    ...
    if (!ctl_initialized) {
		 ...
		 */
        if ((ctl_sarena =
                 arenas_i_impl(tsd, MALLCTL_ARENAS_ALL, false, true)) == NULL) {
            ret = true;
            goto label_return;
        }
        ctl_sarena->initialized = true;

        if ((ctl_darena = arenas_i_impl(tsd, MALLCTL_ARENAS_DESTROYED, false,
                                        true)) == NULL) {
            ret = true;
            goto label_return;
        }
                            ...
                            size_t newlen)
{
    ...
    if (newind != oldind) {
        ...
        if (tcache_available(tsd)) {
            tcache_arena_reassociate(tsd_tsdn(tsd), tsd_tcachep_get(tsd),
                                     newarena);
        }
    }
...
static int arena_i_reset_destroy_helper(tsd_t *tsd, const size_t *mib,
                                        size_t miblen, void *oldp,
                                        size_t *oldlenp, void *newp,
                                        size_t newlen, unsigned *arena_ind,
                                        arena_t **arena)
{
    int ret;
...
static void arena_reset_prepare_background_thread(tsd_t *tsd,
                                                  unsigned arena_ind)
{
                             ...
                             size_t newlen)
{
    ...
    ret = arena_i_reset_destroy_helper(tsd, mib, miblen, oldp, oldlenp, newp,
                                       newlen, &arena_ind, &arena);
    if (ret != 0) {
        return ret;
    }
...
static int arena_i_decay_ms_ctl_impl(tsd_t *tsd, const size_t *mib,
                                     size_t miblen, void *oldp, size_t *oldlenp,
                                     void *newp, size_t newlen, bool dirty)
{
    int ret;
    unsigned arena_ind;
    arena_t *arena;
    ...
    if (newp != NULL) {
        ...
        if (dirty ? arena_dirty_decay_ms_set(tsd_tsdn(tsd), arena,
                                             *(ssize_t *)newp) :
                    arena_muzzy_decay_ms_set(tsd_tsdn(tsd), arena,
                                             *(ssize_t *)newp)) {
            ret = EFAULT;
            goto label_return;
        }
...
static const ctl_named_node_t *arena_i_index(tsdn_t *tsdn, const size_t *mib,
                                             size_t miblen, size_t i)
{
...
CTL_RO_CGEN(config_stats, stats_arenas_i_mapped,
            atomic_load_zu(&arenas_i(mib[2])->astats->astats.mapped,
                           ATOMIC_RELAXED),
            size_t)
CTL_RO_CGEN(config_stats, stats_arenas_i_retained,
            atomic_load_zu(&arenas_i(mib[2])->astats->astats.retained,
                           ATOMIC_RELAXED),
            size_t)
...
CTL_RO_CGEN(config_stats, stats_arenas_i_base,
            atomic_load_zu(&arenas_i(mib[2])->astats->astats.base,
                           ATOMIC_RELAXED),
            size_t)
CTL_RO_CGEN(config_stats, stats_arenas_i_internal,
            atomic_load_zu(&arenas_i(mib[2])->astats->astats.internal,
                           ATOMIC_RELAXED),
            size_t)
...
CTL_RO_CGEN(config_stats, stats_arenas_i_resident,
            atomic_load_zu(&arenas_i(mib[2])->astats->astats.resident,
                           ATOMIC_RELAXED),
            size_t)
 ...
 */
CTL_RO_CGEN(
    config_stats, stats_arenas_i_large_nfills,
    ctl_arena_stats_read_u64(&arenas_i(mib[2])->astats->astats.nmalloc_large),
    uint64_t)
CTL_RO_CGEN(
    config_stats, stats_arenas_i_large_nflushes,
...
static const ctl_named_node_t *stats_arenas_i_bins_j_index(tsdn_t *tsdn,
                                                           const size_t *mib,
                                                           size_t miblen,
                                                           size_t j)
{
...
CTL_RO_CGEN(
    config_stats, stats_arenas_i_lextents_j_nmalloc,
    ctl_arena_stats_read_u64(&arenas_i(mib[2])->astats->lstats[mib[4]].nmalloc),
    uint64_t)
CTL_RO_CGEN(
    config_stats, stats_arenas_i_lextents_j_ndalloc,
...
CTL_RO_CGEN(config_stats, stats_arenas_i_extents_j_ndirty,
            atomic_load_zu(&arenas_i(mib[2])->astats->estats[mib[4]].ndirty,
                           ATOMIC_RELAXED),
            size_t);
CTL_RO_CGEN(config_stats, stats_arenas_i_extents_j_nmuzzy,
            atomic_load_zu(&arenas_i(mib[2])->astats->estats[mib[4]].nmuzzy,
                           ATOMIC_RELAXED),
            size_t);
CTL_RO_CGEN(config_stats, stats_arenas_i_extents_j_nretained,
            atomic_load_zu(&arenas_i(mib[2])->astats->estats[mib[4]].nretained,
                           ATOMIC_RELAXED),
            size_t);
CTL_RO_CGEN(config_stats, stats_arenas_i_extents_j_dirty_bytes,
            atomic_load_zu(&arenas_i(mib[2])->astats->estats[mib[4]].dirty_bytes,
                           ATOMIC_RELAXED),
            size_t);
CTL_RO_CGEN(config_stats, stats_arenas_i_extents_j_muzzy_bytes,
            atomic_load_zu(&arenas_i(mib[2])->astats->estats[mib[4]].muzzy_bytes,
                           ATOMIC_RELAXED),
            size_t);
CTL_RO_CGEN(
    config_stats, stats_arenas_i_extents_j_retained_bytes,
    atomic_load_zu(&arenas_i(mib[2])->astats->estats[mib[4]].retained_bytes,
                   ATOMIC_RELAXED),
    size_t);

static const ctl_named_node_t *stats_arenas_i_extents_j_index(tsdn_t *tsdn,
                                                              const size_t *mib,
                                                              size_t miblen,
                                                              size_t j)
{
                                              ...
                                              size_t newlen)
{
    ...
    extent_util_stats_verbose_get(tsd_tsdn(tsd), ptr, &util_stats->nfree,
                                  &util_stats->nregs, &util_stats->size,
...
static const ctl_named_node_t *experimental_arenas_i_index(tsdn_t *tsdn,
                                                           const size_t *mib,
                                                           size_t miblen,
                                                           size_t i)
{
