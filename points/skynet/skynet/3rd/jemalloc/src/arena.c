 ...
 */
const char *percpu_arena_mode_names[] = { "percpu", "phycpu", "disabled",
                                          "percpu", "phycpu" };
...
void arena_basic_stats_merge(tsdn_t *tsdn, arena_t *arena, unsigned *nthreads,
                             const char **dss, ssize_t *dirty_decay_ms,
                             ssize_t *muzzy_decay_ms, size_t *nactive,
...
void arena_stats_merge(tsdn_t *tsdn, arena_t *arena, unsigned *nthreads,
                       ...
                       size_t *nmuzzy, arena_stats_t *astats,
                       ...
                       arena_stats_extents_t *estats)
{
    ...
    arena_stats_accum_zu(&astats->abandoned_vm,
                         atomic_load_zu(&arena->stats.abandoned_vm,
                                        ATOMIC_RELAXED));
...
static void arena_slab_reg_alloc_batch(extent_t *slab,
                                       const bin_info_t *bin_info, unsigned cnt,
                                       void **ptrs)
{
    arena_slab_data_t *slab_data = extent_slab_data_get(slab);
...
static void arena_slab_reg_dalloc(extent_t *slab, arena_slab_data_t *slab_data,
                                  void *ptr)
{
...
static void arena_large_malloc_stats_update(tsdn_t *tsdn, arena_t *arena,
                                            size_t usize)
{
...
static void arena_large_dalloc_stats_update(tsdn_t *tsdn, arena_t *arena,
                                            size_t usize)
{
    szind_t index, hindex;
...
static void arena_large_ralloc_stats_update(tsdn_t *tsdn, arena_t *arena,
                                            size_t oldusize, size_t usize)
{
...
extent_t *arena_extent_alloc_large(tsdn_t *tsdn, arena_t *arena, size_t usize,
...
void arena_extent_ralloc_large_shrink(tsdn_t *tsdn, arena_t *arena,
                                      extent_t *extent, size_t oldusize)
{
...
static bool arena_decay_deadline_reached(const arena_decay_t *decay,
                                         const nstime_t *time)
{
...
static void arena_decay_backlog_update_last(arena_decay_t *decay,
...
static void arena_decay_backlog_update(arena_decay_t *decay,
...
static void arena_decay_epoch_advance_helper(arena_decay_t *decay,
                                             const nstime_t *time,
                                             size_t current_npages)
{
...
static bool arena_decay_init(arena_decay_t *decay, ssize_t decay_ms,
...
static size_t arena_stash_decayed(tsdn_t *tsdn, arena_t *arena,
                                  extent_hooks_t **r_extent_hooks,
                                  extents_t *extents, size_t npages_limit,
                                  size_t npages_decay_max,
                                  ...
                                  bool is_background_thread)
{
    ...
    if (config_stats) {
        ...
        arena_stats_add_u64(tsdn, &arena->stats, &decay->stats->nmadvise,
                            nmadvise);
                                 ...
                                 bool is_background_thread)
{
    ...
    size_t npurge =
        arena_stash_decayed(tsdn, arena, &extent_hooks, extents, npages_limit,
                            npages_decay_max, &decay_extents);
    if (npurge != 0) {
        size_t npurged =
            arena_decay_stashed(tsdn, arena, &extent_hooks, decay, extents, all,
...
static bool arena_decay_muzzy(tsdn_t *tsdn, arena_t *arena,
                              bool is_background_thread, bool all)
{
...
static void arena_bin_slabs_full_insert(arena_t *arena, bin_t *bin,
                                        extent_t *slab)
{
    assert(extent_nfree_get(slab) == 0);
    /*
...
static void arena_bin_slabs_full_remove(arena_t *arena, bin_t *bin,
                                        extent_t *slab)
{
    if (arena_is_auto(arena)) {
        return;
                                       ...
                                       szind_t szind)
{
    ...
    slab = extent_alloc_wrapper(tsdn, arena, r_extent_hooks, NULL,
                                bin_info->slab_size, 0, PAGE, true, szind,
                                &zero, &commit);
                                  ...
                                  unsigned binshard, const bin_info_t *bin_info)
{
    ...
    extent_t *slab =
        extents_alloc(tsdn, arena, &extent_hooks, &arena->extents_dirty, NULL,
                      bin_info->slab_size, 0, PAGE, true, binind, &zero,
                      &commit);
                             ...
                             uint64_t prof_accumbytes)
{
    ...
    for (i = 0, nfill = (tcache_bin_info[binind].ncached_max >>
        ...
        if ((slab = bin->slabcur) != NULL && extent_nfree_get(slab) > 0) {
            ...
            arena_slab_reg_alloc_batch(slab, &bin_infos[binind], cnt,
                                       tbin->avail - nfill + i);
        } else {
            cnt = 1;
            void *ptr =
...
static void *arena_malloc_small(tsdn_t *tsdn, arena_t *arena, szind_t binind,
                                bool zero)
{
    void *ret;
    bin_t *bin;
                   ...
                   bool zero, tcache_t *tcache)
{
    ...
    if (usize <= SC_SMALL_MAXCLASS &&
        ...
        ret = arena_malloc(tsdn, arena, usize, sz_size2index(usize), zero,
                           tcache, true);
    } else {
...
static void arena_dissociate_bin_slab(arena_t *arena, extent_t *slab,
                                      bin_t *bin)
{
...
static void arena_dalloc_bin_locked_impl(tsdn_t *tsdn, arena_t *arena,
                                         bin_t *bin, szind_t binind,
                                         extent_t *slab, void *ptr, bool junked)
{
    arena_slab_data_t *slab_data = extent_slab_data_get(slab);
    const bin_info_t *bin_info = &bin_infos[binind];
...
static void arena_dalloc_bin(tsdn_t *tsdn, arena_t *arena, extent_t *extent,
                             void *ptr)
{
...
{
    if (alignment == 0) {
        return arena_malloc(tsdn, arena, usize, sz_size2index(usize), zero,
                            tcache, true);
    }
                   ...
                   hook_ralloc_args_t *hook_args)
{
    ...
    if (oldsize >= SC_LARGE_MINCLASS && usize >= SC_LARGE_MINCLASS) {
        return large_ralloc(tsdn, arena, ptr, usize, alignment, zero, tcache,
                            hook_args);
    }
...
arena_t *arena_new(tsdn_t *tsdn, unsigned ind, extent_hooks_t *extent_hooks)
{
    ...
    if (config_cache_oblivious) {
        ...
        atomic_store_zu(&arena->offset_state,
                        config_debug ? ind : (size_t)(uintptr_t)arena,
                        ATOMIC_RELAXED);
    }
