...
static rtree_node_elm_t *rtree_node_alloc_impl(tsdn_t *tsdn, rtree_t *rtree,
                                               size_t nelms)
{
...
static void rtree_node_dalloc_impl(tsdn_t *tsdn, rtree_t *rtree,
...
static rtree_leaf_elm_t *rtree_leaf_alloc_impl(tsdn_t *tsdn, rtree_t *rtree,
                                               size_t nelms)
{
...
static void rtree_leaf_dalloc_impl(tsdn_t *tsdn, rtree_t *rtree,
...
#ifdef JEMALLOC_JET
#if RTREE_HEIGHT > 1
static void rtree_delete_subtree(tsdn_t *tsdn, rtree_t *rtree,
                                 rtree_node_elm_t *subtree, unsigned level)
{
...
static rtree_node_elm_t *rtree_node_init(tsdn_t *tsdn, rtree_t *rtree,
                                         unsigned level, atomic_p_t *elmp)
{
...
static rtree_leaf_elm_t *rtree_leaf_init(tsdn_t *tsdn, rtree_t *rtree,
                                         atomic_p_t *elmp)
{
    malloc_mutex_lock(tsdn, &rtree->init_lock);
    /*
    ...
    if (leaf == NULL) {
        leaf = rtree_leaf_alloc(tsdn, rtree,
                                ZU(1) << rtree_levels[RTREE_HEIGHT - 1].bits);
        if (leaf == NULL) {
            malloc_mutex_unlock(tsdn, &rtree->init_lock);
            return NULL;
        }
...
static rtree_node_elm_t *rtree_child_node_tryread(rtree_node_elm_t *elm,
                                                  bool dependent)
{
    rtree_node_elm_t *node;
...
static rtree_leaf_elm_t *rtree_child_leaf_tryread(rtree_node_elm_t *elm,
                                                  bool dependent)
{
    rtree_leaf_elm_t *leaf;
...
#define RTREE_GET_CHILD(level)                                                 \
    {                                                                          \
        ...
        if (level + 2 < RTREE_HEIGHT) {                                        \
            node = init_missing ?                                              \
                       rtree_child_node_read(tsdn, rtree, &node[subkey],       \
                                             level, dependent) :               \
                       rtree_child_node_tryread(&node[subkey], dependent);     \
        } else {                                                               \
            leaf = init_missing ?                                              \
                       rtree_child_leaf_read(tsdn, rtree, &node[subkey],       \
                                             level, dependent) :               \
