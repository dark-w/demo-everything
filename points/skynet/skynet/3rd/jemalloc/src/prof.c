...
static size_t prof_log_bt_index(tsd_t *tsd, prof_bt_t *bt)
{
    ...
    if (ckh_search(&log_bt_node_set, (void *)(&dummy_node), (void **)(&node),
                   NULL)) {
...
static size_t prof_log_thr_index(tsd_t *tsd, uint64_t thr_uid, const char *name)
{
    ...
    if (ckh_search(&log_thr_node_set, (void *)(&dummy_node), (void **)(&node),
                   NULL)) {
...
static void prof_try_log(tsd_t *tsd, const void *ptr, size_t usize,
                         prof_tctx_t *tctx)
{
...
static _Unwind_Reason_Code prof_unwind_callback(struct _Unwind_Context *context,
                                                void *arg)
{
...
static bool prof_lookup_global(tsd_t *tsd, prof_bt_t *bt, prof_tdata_t *tdata,
                               void **p_btkey, prof_gctx_t **p_gctx,
                               bool *p_new_gctx)
{
    union {
        prof_gctx_t *p;
...
void prof_sample_threshold_update(prof_tdata_t *tdata)
{
    ...
    uint64_t bytes_until_sample =
        (uint64_t)(log(u) / log(1.0 - (1.0 / (double)((uint64_t)1U
                                                      << lg_prof_sample)))) +
        (uint64_t)1U;
...
#ifdef JEMALLOC_JET
static prof_tdata_t *prof_tdata_count_iter(prof_tdata_tree_t *tdatas,
                                           prof_tdata_t *tdata, void *arg)
{
...
static void prof_tctx_merge_tdata(tsdn_t *tsdn, prof_tctx_t *tctx,
                                  prof_tdata_t *tdata)
{
    malloc_mutex_assert_owner(tsdn, tctx->tdata->lock);
...
static void prof_tctx_merge_gctx(tsdn_t *tsdn, prof_tctx_t *tctx,
                                 prof_gctx_t *gctx)
{
...
static prof_tctx_t *prof_tctx_merge_iter(prof_tctx_tree_t *tctxs,
...
static prof_tctx_t *prof_tctx_dump_iter(prof_tctx_tree_t *tctxs,
                                        prof_tctx_t *tctx, void *opaque)
{
...
static prof_tctx_t *prof_tctx_finish_iter(prof_tctx_tree_t *tctxs,
                                          prof_tctx_t *tctx, void *arg)
{
    tsdn_t *tsdn = (tsdn_t *)arg;
    prof_tctx_t *ret;
...
static void prof_dump_gctx_prep(tsdn_t *tsdn, prof_gctx_t *gctx,
...
static prof_gctx_t *prof_gctx_merge_iter(prof_gctx_tree_t *gctxs,
                                         prof_gctx_t *gctx, void *opaque)
{
        ...
        malloc_mutex_lock(tsd_tsdn(tsd), gctx->lock);
        {
            ...
            do {
                prof_tctx_t *to_destroy =
                    tctx_tree_iter(&gctx->tctxs, next, prof_tctx_finish_iter,
                                   (void *)tsd_tsdn(tsd));
                if (to_destroy != NULL) {
                    next = tctx_tree_next(&gctx->tctxs, to_destroy);
                    tctx_tree_remove(&gctx->tctxs, to_destroy);
                    idalloctm(tsd_tsdn(tsd), to_destroy, NULL, NULL, true,
                              true);
                } else {
                    next = NULL;
                }
...
static prof_tdata_t *prof_tdata_merge_iter(prof_tdata_tree_t *tdatas,
                                           prof_tdata_t *tdata, void *opaque)
{
...
static prof_tdata_t *prof_tdata_dump_iter(prof_tdata_tree_t *tdatas,
                                          prof_tdata_t *tdata, void *arg)
{
...
static bool prof_dump_header_impl(tsdn_t *tsdn, bool propagate_err,
 ...
 */
static void prof_leakcheck(const prof_cnt_t *cnt_all, size_t leak_ngctx,
                           const char *filename)
{
#ifdef JEMALLOC_PROF
    /*
...
static prof_gctx_t *prof_gctx_dump_iter(prof_gctx_tree_t *gctxs,
                                        prof_gctx_t *gctx, void *opaque)
{
    prof_gctx_t *ret;
...
static bool prof_dump(tsd_t *tsd, bool propagate_err, const char *filename,
                      bool leakcheck)
{
    cassert(config_prof);
    ...
    bool err =
        prof_dump_file(tsd, propagate_err, filename, leakcheck, tdata,
...
static void prof_dump_filename(char *filename, char v, uint64_t vseq)
{
    ...
    if (vseq != VSEQ_INVALID) {
        ...
        malloc_snprintf(filename, DUMP_FILENAME_BUFSIZE,
                        "%s.%d.%" FMTu64 ".%c.heap", opt_prof_prefix,
                                          ...
                                          char *thread_name, bool active)
{
    ...
    tdata = (prof_tdata_t *)iallocztm(tsd_tsdn(tsd), sizeof(prof_tdata_t),
                                      sz_size2index(sizeof(prof_tdata_t)),
                                      false, NULL, true,
                                      arena_get(TSDN_NULL, 0, true), true);
    if (tdata == NULL) {
        return NULL;
...
static bool prof_tdata_should_destroy_unlocked(prof_tdata_t *tdata,
                                               bool even_if_attached)
{
...
static void prof_tdata_destroy_locked(tsd_t *tsd, prof_tdata_t *tdata,
                                      bool even_if_attached)
{
...
static prof_tdata_t *prof_tdata_reset_iter(prof_tdata_tree_t *tdatas,
                                           prof_tdata_t *tdata, void *arg)
{
    tsdn_t *tsdn = (tsdn_t *)arg;
...
static void prof_log_emit_metadata(emitter_t *emitter)
{
    ...
    emitter_json_kv(emitter, "lg_sample_rate", emitter_type_int,
                    &lg_prof_sample);
...
bool prof_boot2(tsd_t *tsd)
{
    ...
    if (opt_prof) {
        ...
        gctx_locks = (malloc_mutex_t *)base_alloc(
            tsd_tsdn(tsd), b0get(), PROF_NCTX_LOCKS * sizeof(malloc_mutex_t),
            CACHELINE);
        if (gctx_locks == NULL) {
            return true;
        }
        ...
        tdata_locks = (malloc_mutex_t *)base_alloc(
            tsd_tsdn(tsd), b0get(), PROF_NTDATA_LOCKS * sizeof(malloc_mutex_t),
            CACHELINE);
        if (tdata_locks == NULL) {
            return true;
        }
