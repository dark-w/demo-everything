...
bool background_thread_create(tsd_t *tsd, unsigned arena_ind) NOT_REACHED
    ...
    bool background_thread_stats_read(tsdn_t *tsdn,
        ...
        NOT_REACHED void background_thread_ctl_init(tsdn_t *tsdn) NOT_REACHED
#undef NOT_REACHED
#else

static bool background_thread_enabled_at_fork;

static void background_thread_info_init(tsdn_t *tsdn,
...
static inline size_t decay_npurge_after_interval(arena_decay_t *decay,
                                                 size_t interval)
{
    size_t i;
    uint64_t sum = 0;
...
static uint64_t arena_decay_compute_purge_interval_impl(tsdn_t *tsdn,
                                                        arena_decay_t *decay,
                                                        extents_t *extents)
{
...
static void background_thread_sleep(tsdn_t *tsdn,
                                    background_thread_info_t *info,
                                    uint64_t interval)
{
    if (config_stats) {
...
static bool background_thread_pause_check(tsdn_t *tsdn,
                                          background_thread_info_t *info)
{
...
static inline void background_work_sleep_once(tsdn_t *tsdn,
                                              background_thread_info_t *info,
                                              unsigned ind)
{
...
static bool background_threads_disable_single(tsd_t *tsd,
                                              background_thread_info_t *info)
{
...
static int background_thread_create_signals_masked(
...
static void background_thread0_work(tsd_t *tsd)
{
    ...
    while (background_thread_info[0].state != background_thread_stopped) {
        ...
        background_work_sleep_once(tsd_tsdn(tsd), &background_thread_info[0],
                                   0);
    }
...
bool background_thread_boot1(tsdn_t *tsdn)
{
    ...
    background_thread_info = (background_thread_info_t *)base_alloc(
        tsdn, b0get(),
        opt_max_background_threads * sizeof(background_thread_info_t),
        CACHELINE);
