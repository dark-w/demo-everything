...
static const char *optstring(struct skynet_context *ctx, const char *key,
                             const char *str)
{
                   ...
                   const char *args, size_t sz)
{
    ...
    const char *path =
        optstring(ctx, "lua_path", "./lualib/?.lua;./lualib/?/init.lua");
    ...
    if (lua_getfield(L, LUA_REGISTRYINDEX, "memlimit") == LUA_TNUMBER) {
        ...
        l->mem_limit = limit;
        skynet_error(ctx, "Set memory limit to %.2f M",
...
static int launch_cb(struct skynet_context *context, void *ud, int type,
