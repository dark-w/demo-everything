                                    ...
                                    const char name[GLOBALNAME_LENGTH])
{
    ...
    while (node) {
        if (node->hash == h &&
            strncmp(node->key, name, GLOBALNAME_LENGTH) == 0) {
            return node;
        }
        node = node->next;
    }
...
static void forward_local_messsage(struct harbor *h, void *msg, int sz)
{
    ...
    if (skynet_send(h->ctx, header.source, destination,
                    type | PTYPE_TAG_DONTCOPY, (int)header.session, (void *)msg,
            ...
            if (harbor_id == (h->slave >> HANDLE_REMOTE_SHIFT)) {
                ...
                while ((m = pop_queue(s->queue)) != NULL) {
                    ...
                    skynet_send(context, m->header.source, handle,
                                type | PTYPE_TAG_DONTCOPY, m->header.session,
                                m->buffer, m->size);
                }
                release_queue(s->queue);
                s->queue = NULL;
    ...
    for (;;) {
        ...
        case STATUS_HEADER: {
            ...
            if (size < need) {
                ...
                if (s->size[0] != 0) {
                    skynet_error(h->ctx, "Message is too long from harbor %d",
                                 id);
                    close_harbor(h, id);
                    return;
                }
                              ...
                              const char *msg, size_t sz)
{
    ...
    if (s->fd == 0 || s->status == STATUS_HANDSHAKE) {
        ...
        cookie.destination = (destination & HANDLE_MASK) |
                             ((uint32_t)type << HANDLE_REMOTE_SHIFT);
                            ...
                            int session, const char *msg, size_t sz)
{
    ...
    if (node->value == 0) {
        ...
        return remote_send_handle(h, source, node->value, type, session, msg,
                                  sz);
    }
}
                    ...
                    int session, uint32_t source, const void *msg, size_t sz)
{
        ...
        case SKYNET_SOCKET_TYPE_WARNING: {
            ...
            if (id) {
                skynet_error(context,
                             "message havn't send to Harbor (%d) reach %d K",
                             id, message->ud);
            }
            break;
        }
    ...
    case PTYPE_SYSTEM: {
        ...
        if (rmsg->destination.handle == 0) {
            ...
            if (remote_send_handle(h, source, rmsg->destination.handle,
                                   rmsg->type, session, rmsg->message,
                                   rmsg->sz)) {
                return 0;
            }
        }
    ...
    default:
        skynet_error(context, "recv invalid message from %x,  type = %d",
                     source, type);
