...
struct message_queue *skynet_context_message_dispatch(struct skynet_monitor *sm,
                                                      ...
                                                      int weight)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (overload) {
            skynet_error(ctx, "May overload, message queue length = %d",
                         overload);
        }
...
static void handle_exit(struct skynet_context *context, uint32_t handle)
{
    ...
    if (G_NODE.monitor_exit) {
        skynet_send(context, handle, G_NODE.monitor_exit, PTYPE_CLIENT, 0, NULL,
                    0);
    }
    skynet_handle_retire(handle);
}
