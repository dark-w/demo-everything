...
static void fill_header(lua_State *L, uint8_t *buf, int sz)
{
    assert(sz < 0x10000);
    buf[0] = (sz >> 8) & 0xff;
    buf[1] = sz & 0xff;
...
/*
	...
	trace
		...
		STRING tag
 */
static int packreq_number(lua_State *L, int session, void *msg, uint32_t sz,
                          int is_push)
{
                          ...
                          int is_push)
{
    ...
    if (name == NULL || namelen < 1 || namelen > 255) {
        skynet_free(msg);
        if (name == NULL) {
            luaL_error(L, "name is not a string, it's a %s",
...
static void packreq_multi(lua_State *L, int session, void *msg, uint32_t sz)
{
    uint8_t buf[TEMP_LENGTH];
    int part = (sz - 1) / MULTI_PART + 1;
    int i;
...
static int lpacktrace(lua_State *L)
{
    size_t sz;
...
static int unpackmreq_number(lua_State *L, const uint8_t *buf, int sz,
                             int is_push)
{
    if (sz != 13) {
        return luaL_error(
            L, "Invalid cluster message size %d (multi req must be 13)", sz);
    }
    uint32_t address = unpack_uint32(buf + 1);
    uint32_t session = unpack_uint32(buf + 5);
...
static int unpacktrace(lua_State *L, const char *buf, int sz)
{
...
static int unpackmreq_string(lua_State *L, const uint8_t *buf, int sz,
                             int is_push)
{
    if (sz < 2) {
        return luaL_error(L, "Invalid cluster message (size=%d)", sz);
...
LUAMOD_API int luaopen_skynet_cluster_core(lua_State *L)
{
    luaL_Reg l[] = {
