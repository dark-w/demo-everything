...
static inline const struct table *gettable(const struct document *doc,
                                           int index)
{
    ...
    return (const struct table *)((const char *)doc + sizeof(uint32_t) +
                                  sizeof(uint32_t) + doc->n * sizeof(uint32_t) +
                                  doc->index[index]);
}

static void create_proxy(lua_State *L, const void *data, int index)
{
...
static void pushvalue(lua_State *L, const void *v, int type,
...
static void copytable(lua_State *L, int tbl, struct proxy *p)
{
    ...
    const uint32_t *v =
        (const uint32_t *)((const char *)t + sizeof(uint32_t) +
                           sizeof(uint32_t) + ((t->array + t->dict + 3) & ~3));
    int i;
    for (i = 0; i < t->array; i++) {
...
LUAMOD_API int luaopen_skynet_datasheet_core(lua_State *L)
{
    luaL_checkversion(L);
    luaL_Reg l[] = {
        { "new", lnew },
