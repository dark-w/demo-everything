...
static void mark_shared(lua_State *L)
{
    if (lua_type(L, -1) != LUA_TTABLE) {
        luaL_error(L, "Not a table, it's a %s.",
...
{
    struct state_ud *ud =
        (struct state_ud *)luaL_checkudata(L, 1, "BOXMATRIXSTATE");
    if (ud->L) {
        lua_close(ud->L);
        ud->L = NULL;
    }
...
{
    struct state_ud *ud =
        (struct state_ud *)lua_newuserdatauv(L, sizeof(*ud), 0);
    ud->L = mL;
...
LUAMOD_API int luaopen_skynet_sharetable_core(lua_State *L)
{
    luaL_checkversion(L);
    luaL_Reg l[] = {
        { "clone", clone_table },
