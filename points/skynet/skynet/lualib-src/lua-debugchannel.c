...
static struct channel *channel_new()
{
...
static struct channel *channel_connect(struct channel *c)
{
...
static struct channel *channel_release(struct channel *c)
{
    SPIN_LOCK(c)
    --c->ref;
    if (c->ref > 0) {
...
static struct command *channel_read(struct channel *c, double timeout)
{
...
static void channel_write(struct channel *c, const char *s, size_t sz)
{
...
static struct channel *new_channel(lua_State *L, struct channel *c)
{
    if (c == NULL) {
        c = channel_new();
    } else {
...
LUAMOD_API int luaopen_skynet_debugchannel(lua_State *L)
{
    luaL_Reg l[] = {
        { "create", lcreate }, // for write
        { "connect", lconnect }, // for read
        { "release", lrelease }, { "sethook", db_sethook }, { NULL, NULL },
    };
    luaL_checkversion(L);
    luaL_newlib(L, l);
    return 1;
