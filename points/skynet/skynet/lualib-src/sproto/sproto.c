...
static void *pool_newchunk(struct pool *p, size_t sz)
{
...
static inline int toword(const uint8_t *p)
{
    return p[0] | p[1] << 8;
}

static inline uint32_t todword(const uint8_t *p)
{
...
static const char *import_string(struct sproto *s, const uint8_t *stream)
{
...
static int calc_pow(int base, int n)
{
    int r;
    if (n == 0)
        return 1;
...
static const uint8_t *import_field(struct sproto *s, struct field *f,
...
*/
static const uint8_t *import_type(struct sproto *s, struct sproto_type *t,
                                  const uint8_t *stream)
{
    const uint8_t *result;
    uint32_t sz = todword(stream);
...
*/
static const uint8_t *import_protocol(struct sproto *s, struct protocol *p,
                                      const uint8_t *stream)
{
    const uint8_t *result;
    uint32_t sz = todword(stream);
...
static struct sproto *create_from_bundle(struct sproto *s,
                                         const uint8_t *stream, size_t sz)
{
...
void sproto_release(struct sproto *s)
{
    if (s == NULL)
        return;
    pool_release(&s->memory);
...
static const char *get_typename(int type, struct field *f)
{
...
void sproto_dump(struct sproto *s)
{
    int i, j;
    printf("=== %d types ===\n", s->type_n);
    for (i = 0; i < s->type_n; i++) {
    ...
    for (i = 0; i < s->protocol_n; i++) {
        ...
        if (p->p[SPROTO_REQUEST]) {
            printf("\t%s (%d) request:%s", p->name, p->tag,
                   p->p[SPROTO_REQUEST]->name);
        } else {
...
// query
int sproto_prototag(const struct sproto *sp, const char *name)
{
    int i;
...
static struct protocol *query_proto(const struct sproto *sp, int tag)
{
...
struct sproto_type *sproto_protoquery(const struct sproto *sp, int proto,
                                      int what)
{
    struct protocol *p;
    if (what < 0 || what > 1) {
...
int sproto_protoresponse(const struct sproto *sp, int proto)
{
    struct protocol *p = query_proto(sp, proto);
    return (p != NULL && (p->p[SPROTO_RESPONSE] || p->confirm));
}

const char *sproto_protoname(const struct sproto *sp, int proto)
{
    struct protocol *p = query_proto(sp, proto);
    if (p) {
        return p->name;
...
const char *sproto_name(struct sproto_type *st)
{
    return st->name;
}

static struct field *findtag(const struct sproto_type *st, int tag)
{
    int begin, end;
    if (st->base >= 0) {
        tag -= st->base;
...
static inline int fill_size(uint8_t *data, int sz)
{
    data[0] = sz & 0xff;
    data[1] = (sz >> 8) & 0xff;
    data[2] = (sz >> 16) & 0xff;
...
static int encode_integer(uint32_t v, uint8_t *data, int size)
{
...
static int encode_object(sproto_callback cb, struct sproto_arg *args,
...
static inline void uint32_to_uint64(int negative, uint8_t *buffer)
{
    if (negative) {
        buffer[4] = 0xff;
        buffer[5] = 0xff;
...
static uint8_t *encode_integer_array(sproto_callback cb,
                                     struct sproto_arg *args, uint8_t *buffer,
                                     int size, int *noarray)
{
    ...
    for (;;) {
        ...
        if (sz == SIZEOF_INT32) {
            ...
            if (intlen == SIZEOF_INT32) {
                ...
                for (i = index - 2; i >= 0; i--) {
                    int negative;
                    memcpy(header + 1 + i * SIZEOF_INT64,
...
static uint8_t *encode_array_object(sproto_callback cb, struct sproto_arg *args,
                                    uint8_t *buffer, int size, int *noarray)
{
    int sz;
    *noarray = 0;
    args->index = 1;
...
static int encode_array(sproto_callback cb, struct sproto_arg *args,
                        uint8_t *data, int size)
{
    uint8_t *buffer;
    int sz;
...
int sproto_encode(const struct sproto_type *st, void *buffer, int size,
...
static int decode_array_object(sproto_callback cb, struct sproto_arg *args,
                               uint8_t *stream, int sz)
{
    uint32_t hsz;
    int index = 1;
    while (sz > 0) {
...
static inline uint64_t expand64(uint32_t v)
{
    uint64_t value = v;
    if (value & 0x80000000) {
        value |= (uint64_t)~0 << 32;
...
static int decode_array(sproto_callback cb, struct sproto_arg *args,
                        uint8_t *stream)
{
    uint32_t sz = todword(stream);
    int type = args->type;
...
int sproto_decode(const struct sproto_type *st, const void *data, int size,
                  sproto_callback cb, void *ud)
{
    struct sproto_arg args;
    int total = size;
        ...
        if (value < 0) {
            if (f->type & SPROTO_TARRAY) {
                ...
                case SPROTO_TINTEGER: {
                    ...
                    if (sz == SIZEOF_INT32) {
                        uint64_t v =
                            expand64(todword(currentdata + SIZEOF_LENGTH));
                        args.value = &v;
                        args.length = sizeof(v);
                        cb(&args);
                    } else if (sz != SIZEOF_INT64) {
                        ...
                        uint32_t hi =
                            todword(currentdata + SIZEOF_LENGTH + SIZEOF_INT32);
...
static int pack_seg(const uint8_t *src, uint8_t *buffer, int sz, int n)
{
    uint8_t header = 0;
    int notzero = 0;
    int i;
...
static inline void write_ff(const uint8_t *src, const uint8_t *src_end,
                            uint8_t *des, int n)
{
    des[0] = 0xff;
    des[1] = n - 1;
...
int sproto_pack(const void *srcv, int srcsz, void *bufferv, int bufsz)
{
    uint8_t tmp[8];
    int i;
...
int sproto_unpack(const void *srcv, int srcsz, void *bufferv, int bufsz)
{
    const uint8_t *src = srcv;
    uint8_t *buffer = bufferv;
    int size = 0;
