...
static void lua_geti(lua_State *L, int index, lua_Integer i)
{
...
static void lua_seti(lua_State *L, int index, lua_Integer n)
{
    index = lua_absindex(L, index);
    lua_pushinteger(L, n);
    lua_insert(L, -2);
...
static int64_t tointegerx(lua_State *L, int idx, int *isnum)
{
    int64_t v;
    if (lua_isnumber(L, idx)) {
...
static int tobooleanx(lua_State *L, int idx, int *isbool)
{
    if (isbool)
        *isbool = 1;
...
{
    const char *str = luaL_tolstring(
        L, idx, len); // call metamethod, '__tostring' must return a string
    if (isstring) {
        *isstring = 1;
    }
    lua_pop(L, 1);
...
static int lquerytype(lua_State *L)
{
...
static int next_list(lua_State *L, struct encode_ud *self)
{
...
static int get_encodefield(const struct sproto_arg *args)
{
    ...
    if (args->index > 0) {
        ...
        if (args->tagname != self->array_tag) {
            ...
            if (luaL_getmetafield(L, self->array_index, "__pairs")) {
                ...
                return luaL_error(
                    L,
                    ".*%s(%d) should be a table or an userdata with metamethods (Is a %s)",
                    args->tagname, args->index,
...
static int encode_one(const struct sproto_arg *args, struct encode_ud *self)
{
    lua_State *L = self->L;
    int type = args->type;
    switch (type) {
    ...
    case SPROTO_TSTRING: {
        ...
        int type = lua_type(
            L,
            -1); // get the type firstly, lua_tolstring may convert value on stack to string
    ...
    case SPROTO_TSTRUCT: {
        ...
        sub.iter_table = 0;
        sub.iter_key = 0;
        r = sproto_encode(args->subtype, args->value, args->length, encode,
                          &sub);
...
static void *expand_buffer(lua_State *L, int osz, int nsz)
{
    void *output;
    do {
        osz *= 2;
...
static int decode(const struct sproto_arg *args)
{
    ...
    case SPROTO_TSTRUCT: {
        ...
        if (args->mainindex >= 0) {
            ...
            r = sproto_decode(args->subtype, args->value, args->length, decode,
                              &sub);
            if (r < 0)
                return SPROTO_CB_ERROR;
            if (r != args->length)
            ...
            if (map) {
                ...
                if (lua_isnil(L, -1)) {
                    luaL_error(L, "Can't find key field in [%s]",
                               args->tagname);
                }
                lua_getfield(L, sub.result_index, args->vtagname);
                if (lua_isnil(L, -1)) {
                    luaL_error(L, "Can't find value field in [%s]",
                               args->tagname);
                }
                ...
                if (lua_isnil(L, -1)) {
                    luaL_error(L, "Can't find main index (tag=%d) in [%s]",
                               args->mainindex, args->tagname);
                }
...
static void pushfunction_withbuffer(lua_State *L, const char *name,
                                    lua_CFunction func)
{
...
static int lprotocol(lua_State *L)
{
    struct sproto *sp = lua_touserdata(L, 1);
...
static int lsaveproto(lua_State *L)
{
    struct sproto *sp = lua_touserdata(L, 1);
...
static int lloadproto(lua_State *L)
{
...
static void push_default(const struct sproto_arg *args, int table)
{
    lua_State *L = args->ud;
    switch (args->type) {
    case SPROTO_TINTEGER:
 ...
 */
static int ldefault(lua_State *L)
{
    int ret;
...
#endif
    luaL_Reg l[] = {
        { "newproto", lnewproto },   { "deleteproto", ldeleteproto },
        { "dumpproto", ldumpproto }, { "querytype", lquerytype },
        { "decode", ldecode },       { "protocol", lprotocol },
        { "loadproto", lloadproto }, { "saveproto", lsaveproto },
        { "default", ldefault },     { NULL, NULL },
    };
    luaL_newlib(L, l);
