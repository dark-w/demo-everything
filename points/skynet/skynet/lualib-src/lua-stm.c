...
static struct stm_copy *stm_newcopy(void *msg, int32_t sz)
{
...
static struct stm_object *stm_new(void *msg, int32_t sz)
{
...
static void stm_releasecopy(struct stm_copy *copy)
{
    if (copy == NULL)
        return;
...
static struct stm_copy *stm_copy(struct stm_object *obj)
{
    rwlock_rlock(&obj->lock);
...
static int lnewwriter(lua_State *L)
{
    void *msg;
    size_t sz;
...
static int ldeletewriter(lua_State *L)
{
    struct boxstm *box = lua_touserdata(L, 1);
    stm_release(box->obj);
    box->obj = NULL;
...
static int lnewreader(lua_State *L)
{
...
static int ldeletereader(lua_State *L)
{
...
LUAMOD_API int luaopen_skynet_stm(lua_State *L)
{
    ...
    lua_pushcfunction(L, ldeletewriter), lua_setfield(L, -2, "__gc");
    lua_pushcfunction(L, lupdate), lua_setfield(L, -2, "__call");
    ...
    lua_createtable(L, 0, 2);
    lua_pushcfunction(L, ldeletereader), lua_setfield(L, -2, "__gc");
    lua_pushcfunction(L, lread), lua_setfield(L, -2, "__call");
